...
class Simulator:
    ...
    def initialize_state(self, random_init: bool = False, init_range: float = 0.0) -> NDArray[np.float64]:
        ...
        if random_init:
            # Random positions
            s[0:3, :] = np.random.uniform(-init_range, init_range, (3, self.num_quadrotors))
            # Random velocities
            s[3:6, :] = np.random.uniform(-init_range, init_range, (3, self.num_quadrotors))
            # Random orientations
            s[6:9, :] = np.random.uniform(-init_range * np.pi, init_range * np.pi, (3, self.num_quadrotors))
    ...
    def run_simulation(self, initial_state: Optional[NDArray[np.float64]] = None, leader_speed: float = 1.0, 
        ...
        position_history = np.zeros((self.state_dim, self.num_quadrotors + 1, self.num_steps + 1))
        ...
        for t in range(self.num_steps):
            ...
            if self.num_quadrotors == 1:
                ...
                for i in range(self.num_quadrotors):
                    omega = quadrotor_controller(
                        s[:, i], xl, vl, 0, self.parameters, position_gain, velocity_gain
                    )
                    
                    # Update quadrotor state
                    s[:, i] = quadrotor_kinematics(s[:, i], omega, self.parameters, self.dt)
