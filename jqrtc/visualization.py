...
def plot_trajectory(
    ...
    def update(frame: int) -> List[artist.Artist]:
        ...
        ax.plot(
            xy_his_array[0][0, :], xy_his_array[0][1, :], xy_his_array[0][2, :], "kx"
        )

        # Plot leader
        ax.plot(
            xy_his_array[frame][0, 0],
            xy_his_array[frame][1, 0],
            xy_his_array[frame][2, 0],
            "rp",
        )

        # Plot quadrotors
        ax.plot(
            xy_his_array[frame][0, 1:],
            xy_his_array[frame][1, 1:],
            xy_his_array[frame][2, 1:],
            "bo",
            markerfacecolor="b",
            markersize=5,
        )
        ...
        for i in range(1, n):
            ...
            ax.plot(
                xyz[idx, indices], xyz[idx + 1, indices], xyz[idx + 2, indices], "c-"
            )
        ...
        ax.set_title(
            ...
            if title is None
            else f"{title} - Time: {curr_time:.2f}s"
        )
